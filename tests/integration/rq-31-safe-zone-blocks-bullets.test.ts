 ...
 * **원점(0,0,0)**에 두면:
 *   1. 원점은 원의 중심이므로 **모든** 스폰 지점으로부터 정확히 같은
 *      반지름만큼 떨어져 있다(근사가 아니라 원의 정의 그 자체) — 그
 *      반지름은 Safe Zone 반경(4m)보다 압도적으로 크므로(아래 가드가
 *      실측으로 확정) 원점은 항상 모든 Safe Zone 밖이다. 따라서 A 자신의
 *      Safe Zone 위치 기반 발사 불가(GA-19)에 걸리지 않는다.
