 ...
 * 계속 재전송한다.
 *
 * ⚠️ **이 자극은 실 클라이언트가 만들 수 없다 — 서버 내부 불변식 관측용
 * 합성 자극이다.** 초안은 "실 클라이언트가 점프 키를 누르고 있는 동안 매 틱
 * `jump:true`를 다시 보내는 것과 동일하다"고 적었으나 **사실이 아니다**
 * (PR #41 독립 평가 F-A·리뷰 major 1). `src/client/input/movementInput.ts`의
 * `getMoveInput()`이 `jumpPending`을 첫 호출에 소비하고 즉시 내리므로 실
 * 클라는 **키다운당 `jump:true`를 정확히 1회**만 보낸다 — 그 파일 docblock이
 * 원샷 소비가 버니합 방지 **의도**임을 명시한다. 30Hz로 계속 나가는 것은
 * `dirX`·`dirZ`·`mode` 같은 **연속값**이고 엣지 비트인 `jump`는 예외다.
 *
 * 따라서 원장 22f 이후 **제품에서 버니합은 불가능하다.** 이 테스트가 그것을
 * 합성하는 것은 F1의 표적이 제품 행동이 아니라 **서버 내부 불변식**
 * ("착지 전이 그 자체에서 데미지가 동기 적용된다")이기 때문이며, 그 검출력은
 * M1 변이로 검증됐다(평가·리뷰 각각 독립 실측). 이 문면을 정확히 두는 이유:
 * "실 클라가 매 틱 재전송한다"를 믿으면 `GameRoom.ts`의 **리셋을 빼도 된다고
 * 오판**하게 되고, 그러면 한 번의 키다운이 무한 버니합이 된다.
 *
 ...
 * ⚠️ 이는 **실 클라이언트가 만들 수 없는 합성 자극**이다(실 클라는 키다운당
 * 1회만 보낸다 — 파일 상단 REV9 절 참고). 서버 내부 불변식을 관측하기 위한
 * 것이며 제품 행동을 재현하는 것이 아니다. 원장 22f 수정 이후 `pendingInputs.jump`는
