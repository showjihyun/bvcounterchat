 ...
 *
 * **제품 관찰(팀리드가 원장에 이월 등재한 기록) — 서로 다른 두 실측을
 * 혼동하지 않도록 구분한다. (a)·(b) 모두 이후 해소됐다**(아래 각 절의
 * "해소" 문단 참고 — (a)는 v1.5 반경 개정, (b)는 방사-방향 설계 대체).
 * 해소된 뒤에도 지우지 않는 이유: 무엇이 관찰됐고 왜·언제 해소됐는지가
 * 남아야 이 저장소의 "왜 이렇게 됐는지" 관행에 맞고, 다음에 같은 문제를
 * 처음부터 다시 재발견하지 않는다:
 * (a) **자연 겹침**(정지 상태, 이동 전혀 없음) — **v1.5(반경 4m)로 해소됨,
 * 원장 26s**. 반경 5m 시절의 관찰: 반지름 22m 원 위 15개 스폰 지점의
 * 이론상 인접 간격은 `2×22×sin(π/15)`=9.15m인데 Safe Zone 지름이 10m(반경
 * 5m×2)라 9.15m<10m — 인접 스폰끼리 Safe Zone이 겹쳤다. 정수 반올림된
 * 실제 좌표로 재확인: **인접 15쌍 중 14쌍이 겹침**(거리 8.60~9.49m, 전부
 * <10m) — 유일한 예외는 지점 7·8(각각 `{x:-22,z:5}`·`{x:-22,z:-5}`)로
 * 정확히 10.00m(접함, 겹침 아님). 즉 **15개 스폰 지점 전부가 적어도 한
 * 인접 지점과 Safe Zone이 겹쳤다**(7·8도 각자의 다른 이웃과는 겹쳤다) —
 * 맵 가장자리를 따라 사실상 전 구간이 연결된 고리를 이뤘다. **해소**:
 * RQ-31 v1.5(원장 26s, 커밋 `3996402`)가 반경을 4m로 낮추면서
 * `SAFE_ZONE_RADIUS_M`이 5→4가 됐고, 모든 쌍의 최소 거리 8.6023m가 새
 * 지름 8m(반경 4m×2)를 초과해 **겹침·접함이 0건**이 됐다 — 105쌍 전수를
 * `tests/unit/sim-spawn.test.ts`의 GA-51 비겹침 절(커밋 `2c1c5e9`)이
 * 검증한다.
 ...
 * 문제이지 스폰 지점끼리의 자연 겹침이 아니며, 이미 방사-방향(radial
 * -outward) 설계(위 "경위" 절)로 대체되어 해소됐다.
 * (a)는 RQ-31 v1.5가 명문화한 "서로 다른 스폰 지점의 Safe Zone은
 * 겹치지도 접하지도 않아야 한다"가 직접 다루는 대상이었고 반경 축소로
 * 해소됐다. (b)는 애초에 그 조항이 다루는 대상이 아니다 — 정적인 두 원의
 * 겹침이 아니라 이동 중인 플레이어의 궤적 안전성이라, v1.5 이전에도
 * 이후에도 이 조항의 위반이 아니다(별도로 방사-방향 설계가 해소).
 ...
 * 한다 — 그 파일의 전제 가드에 기대지 마라**(가드는 body 개수만 본다).
 *
 * **축을 분리해 서술한다(원장 22y 3차 리뷰 minor 1 — 한 문단에 섞으면
 * 결론이 반대인 두 축이 혼동된다)**:
 *
 * - **오프셋 축**(이 상수 자체가 바뀌는 경우): 0~200m 스윕에서 분류가
 ...
 *   바꾸는 PR은 오프셋 축의 "형식적 확인" 결론을 가져다 쓰지 마라 —
