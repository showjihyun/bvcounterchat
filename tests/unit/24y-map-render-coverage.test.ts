 ...
 *
 * **못 잡는다 — 알려진 두 형태**(평가자 변이 실측):
 * 1. 목록에 이름만 더하고 **JSX 루프를 안 더한** 경우
 * 2. **잘못된 컬렉션을 순회**하는 경우(`platforms` 자리에 `boxes`를 넘김)
 *
 * 둘 다 렌더 계층 안의 일이라 마운트해야 보인다(ADR-0008 §6). 그 짝은 사람이
 * 지키며 `MapMeshes`의 상수 주석에 경고를 박아 두었다. 즉 이 그물은 **완전하지
 * 않고 가장 흔한 누락 형태만** 막는다 — 더 나은 그물(디스패치 표)이 §6 안에서
 * 가능하다는 것이 확인됐고 **원장 24z-2**가 갖는다.
 */
