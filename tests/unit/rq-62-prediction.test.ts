...
/**
 * 예측 버퍼 상한 (리뷰 minor 대응 — `_workspace/review/feat-RQ-62-client-prediction.md`
 * "후속 이슈 권고" 첫 항목). `applyInput`마다 버퍼에 쌓이는 미확인 입력은
 * `reconcile` 호출 시에만 프루닝된다 — 스냅샷이 장시간 끊기고(소켓은 살아
 * 있으나 스냅샷 기아) 입력은 계속되는 병적 상황에서 상한 없이 무한 성장한다.
 *
 * **상한값 근거(재량, 명시)**: 100개. 클라이언트가 서버 틱 레이트(30Hz,
 * `NET.TICK_HZ`)와 동일한 주기로 입력을 보낸다고 가정하면(ADR-0003의
 * "커맨드 버퍼" 설계 전제) 100개는 약 3.3초 분량의 미확인 입력이다. 정상
 * 왕복(수 틱 — 리뷰가 "LAN에서 한 자릿수"로 실측 언급)보다 훨씬 넉넉해
 * 정상 플레이 중 오탐(캡에 걸려 정당한 미확인 입력이 드롭됨) 위험이 없고,
 * 스냅샷 기아 같은 병적 상황에서도 재생 비용(reconcile당 최대 100회
 * stepMovement)과 메모리를 유한하게 묶는다.
 *
 * **관찰 가능한 계약만 테스트한다**: 버퍼 내부 배열 길이를 직접 들여다보지
 * 않는다(private 구현 세부) — 대신 `reconcile`의 **재생 결과**로 상한의
 * 효과를 관찰한다. 상한이 없다면 모든 미확인 입력이 재생되지만, 상한이
 * 있다면 가장 오래된(가장 작은 seq) 초과분이 재생에서 빠진다.
 */
describe(`RQ-62 클라이언트 예측 — 예측 버퍼 상한(리뷰 minor 대응, 스냅샷 기아 시 무한 성장 방지)`, () => {
  ...
  it(`RQ-62/리뷰 minor: 버퍼는 최대 ${BUFFER_CAP}개까지만 보관하고, 초과분은 가장 오래된(가장 작은 seq) 입력부터 드롭한다`, () => {
    ...
    // 서버가 아직 아무 입력도 처리하지 못했다고 가정한다(lastProcessedInputSeq=0)
    // — 버퍼가 무한이라면 seq 1~totalInputs 전부가 재생 대상이지만, 상한이
    // 있다면 가장 오래된 10개(seq 1~10)는 이미 드롭돼 재생에서 빠진다.
    const snapshot: AuthoritativeMoveState = { ...initial, lastProcessedInputSeq: 0 }
    const reconciled = predictor.reconcile(snapshot)

    // 기대값 — 드롭된 앞 10개(인덱스 0~9)를 건너뛰고, 남은 BUFFER_CAP개
    // (인덱스 10~totalInputs-1, seq 11~totalInputs)만 초기 상태 위에서
    // 재생한 결과.
    ...
    // 대조군 확인 — "드롭이 실제로 일어났다"는 것 자체를 증명한다. 상한이
    // 없다면(현재 구현) reconciled는 totalInputs개 전부를 재생한 값과
    // 같아야 하는데, 이는 위 expected(BUFFER_CAP개만 재생)와 달라야 한다 —
    // 다르지 않으면 이 테스트가 캡의 효과를 증명하지 못한다(우연의 일치와
    // 구분 불가).
