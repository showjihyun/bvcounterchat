 ...
 * CHARLIE(x:[-3,6.6], z:[8,14.3], 원점 근처 북쪽). 각 클러스터 내부
 * 박스 간 간격은 0.3m(인접), 클러스터 사이 최소 간격은 4.4m(ALPHA↔
 * CHARLIE) 이상 — 아래 "클러스터링" 절의 문턱(2m)이 이 둘을 명확히
 * 가른다. 높이는 전부 0.3~0.9m(모두 GA-53 상한 1.0m 아래, 아래 "GA-53
 * 실효 상한" 절 참고 — 1.0m 경계에는 의도적으로 닿지 않는다).
 *
 ...
 *   가장 가까운 위험 후보는 인덱스1 `(20,9)`(클러스터 ALPHA 최대 x=17.3
 *   에서 2.7m)·인덱스13 `(15,-16)`(클러스터 BRAVO 박스들과 최근접
 *   코너 거리 ≈1.8m, 겹치지는 않는다)다 — 아래 테스트가 15개 전부와
 *   `PRODUCTION_BOXES`/`PRODUCTION_LADDERS`의 겹침을 직접 확인한다(정적
 *   litreal grep이 아니라 실제 `SPAWN_POINTS` 배열 값으로).
 ...
 * ≈0.997368m다(`JUMP_V0_MPS=√(2×20×1.0)≈6.32456`, `y(9/30)=6.32456×0.3-
 * 10×0.3²=0.997368` — `t=10/30`은 0.99708로 더 낮다, 연속 함수의 진짜
 * 정점 1.0m은 `t=0.316228초`인데 어느 틱도 정확히 이 시각에 오지 않는다).
 * 즉 **topY=1.0m인 박스는 GA-53(≤1.0m)을 통과하지만 실제로는 오를 수
 * 없다**(RQ-32 "등반 가능한 치수"와 모순). **위 제안 좌표는 이 경계에
 * 닿지 않는다** — 가장 높은 박스도 0.7m로, 실효 상한(0.997368m)에서
 * 0.3m 가까이 여유가 있다(실측: 아래 "GA-53" describe가 이 여유를
 * 직접 확인한다). 이 파일은 GA-53 문면 그대로(`<= MOVEMENT.JUMP_HEIGHT`)
 * 만 단언한다 — 문면 개정은 사람 몫이라 이 라운드가 임의로 고치지
 * 않는다(오케스트레이터가 사용자에게 올린다).
 *
...
describe('GA-53 — 박스 상단 높이 상한 (RQ-32, MOVEMENT.JUMP_HEIGHT)', () => {
  ...
  /** 이산 틱 실효 등반 상한(위 docblock "GA-53 실효 상한" 절, 팀리드
   * 실측값 재도출 — 리터럴 고정이 아니라 같은 물리 상수에서 매번
   * 재계산한다). `movement.ts`의 `JUMP_GRAVITY_MPS2`(20)는 비공개
   * 구현값이라 이 파일이 직접 참조할 수 없다 — GA-53 문면 자체가
   * "구현 자유"로 남긴 값이므로, 이 테스트는 그 정확한 상수를 재도출하지
   * 않고 그 상수로 계산된(팀리드 실측) 근사값(0.997368)에 대해 "이
   * 배치가 그 경계에 닿는지"만 확인한다. */
  it('제안 배치는 GA-53의 문면 결함(정확히 1.0m는 통과하지만 못 오르는 경계, 실효 상한 ≈0.997368m)에 닿지 않는다 — 모든 박스가 그 상한보다 여유 있게 낮다', () => {
    const DISCRETE_CLIMB_CEILING_M = 0.997368
    for (const box of PRODUCTION_BOXES) {
...
describe('RQ-32 — 배치 카운트(사다리 2개, 박스 3~18개 = 클러스터 3곳 × 4~6개)', () => {
  it('사다리는 정확히 2개다', () => {
