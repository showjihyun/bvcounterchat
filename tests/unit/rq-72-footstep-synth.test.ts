 ...
 * (엔벨로프)·클리핑 여부를 값으로 단언할 수 있다(ADR-0008 §6의 렌더·오디오
 * 면제에 기대지 않는다). 면제 대상은 **재생 배선**(`AudioContext` 생성·
 * `BufferSource` 연결·포인터 락 개방 시점)뿐이다 — 그 배선은 이 파일이
 * 다루지 않는다(아래 "스코프 밖" 절).
 *
 * ⚠️ **이 파일이 검증하지 않는 것**: 「실제로 들리는가」·「사람 귀에 발소리
 * 같은가」는 어느 단언도 확인하지 않는다(ADR-0008 §6 — 렌더·오디오 계층은
 * 단위 테스트 면제). 이 파일이 고정하는 것은 **파형 배열의 구조적 성질**뿐
 ...
 *   — ADR-0008 §6 면제, 배선 계층(`src/client/audio/` 또는
