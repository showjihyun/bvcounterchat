 ...
 * 계약 문면의 "초과분"이라는 단어가 엄밀히는 "전량"과 다를 수 있어
 * **스펙 질문으로도 별도 보고**한다 — 반대 해석(초과분만 버리고 남은
 * maxBacklogTicks는 이후 여러 호출에 걸쳐 정상 clamp로 소화)이 맞다면
 * 이 describe만 좁게 고치면 된다(전체 재작성 불필요).
 *
...
describe('TickDriver — catch-up clamp (20a-1, RQ-60 v1.1, harness/sim/README.md §5)', () => {
  ...
  it(
    '20a-1 항목5 경계값: 밀림이 정확히 maxBacklogTicks(1000ms→30틱)이면 상한을 "넘은" 것이 아니므로 ' +
      '버려지지 않고 여러 호출에 걸쳐 전부 따라잡는다 (계약 "넘으면"은 초과만을 뜻한다고 해석 — 모호하면 스펙 질문 참고)',
    () => {
      const clock = createClock()
      const scheduler = createScheduler(clock)
      const onOverflow = vi.fn()
