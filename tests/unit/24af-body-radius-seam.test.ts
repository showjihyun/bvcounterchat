 ...
 * ---
 *
 * **REV(coder 수정 `a3361f5` 이후 — 스윕 계약을 이 라운드가 소유한
 * 범위로 좁힌다, 원장 24af `11_test-writer_scope.md`)**: 수정 후에도
 * 명제 seam-2(전수 스윕)의 **21건이 계속 Red**였다 — 실측 결과 **전부
 * `origin/main`에도 있는 별개의 선행 결함**이었다(재현 데이터 전문은
 * `11_test-writer_scope.md`에 있다 — 다음 라운드가 스윕을 그대로 복원할
 * 수 있도록 지우지 않고 보고서에 남겼다). 코디네이터 지시대로 **어느
 * 단언 부분집합이 (a) 수정 후 HEAD 통과 (b) 수정 전 HEAD 실패(=blocker를
 * 실제로 잡음) (c) `origin/main` 통과 세 조건을 모두 만족하는지** 격리
 * 워크트리 3자 대조로 실증했다:
 *
 * | 단언 부분집합 | 수정 후 HEAD | 수정 전 HEAD | `origin/main` |
 * |---|---|---|---|
 * | 명제 1(침투 금지) | 0건 발화(전 구간에서 한 번도 안 걸림) | 0건 발화 | 0건 발화 |
 * | 명제 2 — "맨 지면에 닿기도 전, 출발 박스와 달라야 위반" | **21건 Red** | 23건 Red | 24건 Red |
 * | 명제 2 — "맨 지면에 닿은 뒤(재접지 이후), 어떤 박스든 다시 오르면 위반" | **0건(전부 Green)** | **정확히 1건 Red(블로커 재현 그 자체)** | **0건(전부 Green)** |
 *
 * **결론**: 세 조건을 전부 만족하는 것은 **명제 2의 "재접지 이후"
 * 갈래뿐**이다. "맨 지면에 닿기도 전" 갈래는 `origin/main`에도 있는
 * 별개 결함(박스 가장자리를 걸어 나가 인접한 더 낮은 박스로 낙하하는
 * 도중, `boxesBlockingAt`의 높이 임계값을 아직 넘기 전에 수평 위치가
 * 이미 근접면을 지나쳐 버리는 것으로 추정 — 반경과 무관, 확정 조사는
 * 못 했다)의 증상이었다. **명제 1(침투 금지)은 전 구간에서 한 번도
 * 발화하지 않는다** — `groundedOutcome`이 위치와 높이를 같은 틱에
 * 함께 재계산하므로 "침투했지만 아직 낮은 높이" 상태가 이산 틱에서
 * 관측되지 않기 때문으로 보인다(리뷰어가 서술한 "2.0m 진입"은 이 파일의
 * 틱 단위 관측 모델에서는 등반 판정으로 흡수된다). 명제 1은 어느
 * 상태에서도 실패를 만들지 않으므로(=false positive 없음) 안전하게
 * **그대로 둔다**(제거하면 향후 다른 경로의 침투를 잡을 기회를 잃는다 —
 * 지금은 발화하지 않을 뿐 무의미하지 않다). **아래 구현은 "맨 지면에
 * 닿기도 전" 갈래만 스윕에서 제거하고, 나머지는 전부 그대로다** — 명제
 * seam-1("블로커 재현")은 손대지 않았다(이미 이 계약이고 지금 Green).
 */
...
function assertNoEmbedAndNoUnauthorizedClimb(
  ...
  if (!ctx.hasTouchedGround) {
    // ⚠️ **REV(스코프 축소, 원장 24af `11_test-writer_scope.md`) — 이
    // 분기는 더 이상 단언하지 않는다.** 원래는 "아직 맨 지면에 닿지
    // 않았다면 출발 박스와 달라서는 안 된다"를 확인했는데, 격리
    // 워크트리 3자 대조(수정 후 HEAD / 수정 전 HEAD / `origin/main`)
    // 실측 결과 이 분기가 잡는 것은 **`origin/main`에도 있는 별개의
    // 선행 결함**(박스 가장자리를 걸어 나가 인접한 더 낮은 박스로
    // 낙하하는 도중 그 박스의 높이 임계값을 아직 넘기 전에 수평 위치가
    // 이미 근접면을 지나쳐 버리는 것으로 추정 — 반경과 무관)이었다 —
    // 이 라운드(PR #67 blocker)가 만든 회귀가 아니다. 재현 데이터
    // 21건 전문은 `_workspace/24af/11_test-writer_scope.md`에 좌표·
    // 방향·틱까지 전부 남겼다(다음 라운드가 이 분기를 그대로 복원할 수
    // 있도록 정보를 지우지 않았다). 아래 "재접지 이후" 분기만 이
    // 라운드의 계약이다(리뷰어 재현·이 라운드가 실제로 고친 회귀와
    // 정확히 일치 — 격리 3자 대조: 수정 후 HEAD 0건 · 수정 전 HEAD
    // 정확히 1건(블로커 재현 자신) · `origin/main` 0건).
    return
  }
