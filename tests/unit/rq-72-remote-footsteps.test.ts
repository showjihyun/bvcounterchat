 ...
 * 직전 프레임과의 거리를 누적) 두 계산법을 비교했다.
 *
 * ⚠️ **REV(독립 평가 권고 2, 2026-08-10) — 초안의 수치 서술이 이 파일의
 * fixture 성질로 읽혔으나 아니다.** 초안이 인용한 「60fps=5회(11.4769m),
 * 20fps=4회(9.9394m)」는 **코너 16개짜리 zigzag 변형**에서 나온 값이고,
 * 이 파일의 fixture는 **직각 코너 1개**(30틱 +x · 30틱 +z)다. 평가자가
 * 파일 안 fixture로 재현하니 `delayMs=50`·오프셋 0에서 **60fps 12.000m(6회)
 * · 20fps 12.000m(6회)** 로 **발산이 0**이었다 — 위상이 정렬돼 코너(정확히
 * 1000.0ms)가 두 표본점에 다 잡히기 때문이다. 특히 **60fps는 위상 0인 한
 * 어떤 경로에서도 거리를 잃지 않으므로 「60fps=5회」는 서술된 설정에서
 * 도달 불가능한 값**이다.
 *
 * ✅ **그럼에도 함정은 실재하고 이 fixture도 판별력을 갖는다.** 함정 재현:
 * 20fps에 7ms 오프셋을 주면 11.9614m(5회), 23fps면 11.9643m(5회), zigzag면
 * 20fps에서 10.4721m. 그리고 이 fixture는 **폴링 빈도** 축에서 정상적으로
 * 가른다 — 평가자의 변이 M1(누적 전진을 폴링 시점으로 이동)이 아래 GA-83
 * 테스트 **둘 다**를 죽였다(코너를 가로지르는 쪽이 11.2472m→5회).
 * 즉 결함은 구현·테스트가 아니라 **초안의 근거 서술**에 있었다. 이 파일이 검증하는 API(`getFootstepCount`,
