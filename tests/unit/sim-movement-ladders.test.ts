 ...
 * 지오메트리를 **기본값 있는 위치 인자**로 받는다 — 위치 인자 중 하나만
 * 넘기고 다른 하나를 빠뜨려도(예: `walls`만 넘기고 `boxes`를 생략)
 * 타입이 그대로 통과한다. 서버·클라 발산이 이미 **2회 재발**했다(25a-2
 * F3 벽 · 25a-4 박스, `_workspace/RQ-22-box-jump` "F1 선례") — **단,
 * 두 재발의 결함 형태는 다르다**(리뷰 정정): 25a-4(박스)는 정확히 위
 * "부분 누락" 형태였지만, 25a-2(벽)는 **인자 자체를 통째로 생략**한
 * 형태(`stepMovement(predicted, input)`)였다. 후자는 지오메트리가
 * 전혀 필요 없는 2-인자 호출과 타입상 구분되지 않는 **의도된** 하위
 * 호환 경로라, 아래 `StaticGeometry` 계약으로도 여전히 막히지 않는다
 * — 이 계약이 타입으로 닫는 것은 **부분 누락**뿐이다(전체 생략은
 * 여전히 코드 리뷰·배선 통합 테스트의 몫). 사다리를 세 번째 정적
 * 지오메트리 종류로 얹으면서, 위치 인자를 하나 더 늘리는 대신 **단일
 * 값으로 묶어 부분 누락을 타입 에러로 만든다**:
 *
 * ```ts
 ...
 *
 * **트레이드오프였으나 평가 FAIL로 지적돼 수정됨(F1)**: 최초 구현은
 * `grounded: true`를 쓰기 때문에 사다리를 벗어난 순간 아래에 지지면이
 * 없으면(예: 꼭대기 위에 발판이 없음) 자유낙하 궤적 대신 `standingHeight`
 * 로 즉시 스냅(순간이동)하는 것을 수용된 트레이드오프로 문서화했었다.
 * **그 귀속이 틀렸었다** — 원장 25a-7의 "이 라운드가 닫지 않는 것" 목록에
 * 실제로 있는 것은 "사다리에서 이탈하며 **점프**"뿐이고, "발판 없는
 * 꼭대기 이탈"은 그 목록에 없었다. 독립 평가가 이 오귀속을 FAIL(F1,
 * `_workspace/RQ-21-ladder/03_evaluator_report.md`)로 지적했다 — 발판
 * 없이 꼭대기를 이탈하면 즉시 접지 스냅(4↔0 무한 요요)이 되어 RQ-18
 * 낙하 데미지가 전혀 적용되지 않았다. **수정됨**: `stepMovement`는 이제
 * 이탈 시 지지면이 없으면 공중(`grounded: false`) 상태로 전이하고 사다리
 * 수직 속도를 낙하 궤적에 인계한다 — RQ-21 v1.4 "볼륨을 벗어나면 즉시
 * 중력이 복귀한다"가 문면 그대로 성립하고, 실제 낙하가 `GameRoom
 * .trackFallDamage`에 반영된다. 파일 하단의 "F1 재현" describe가 이
 * 동작을 직접 고정한다 — 그 describe와 아래 "꼭대기 이탈" 테스트는 서로
 * 다른 두 케이스를 나눠 맡는다: 아래는 **지지면이 있는** 이탈(접지 유지,
 * 로컬 `BoxAABB` 발판), "F1 재현"은 **지지면이 없는** 이탈(공중 전이·
 * 낙하 데미지)이다 — 후자를 더 이상 "테스트하지 않는다"고 말하지 않는다.
 *
...
describe('RQ-21 사다리 이동 — 순수 틱 함수 주입 계약 (GA-54 뒷받침, 골든 자체 검증은 통합 레벨)', () => {
  ...
  describe('볼륨을 벗어나면 즉시 중력이 복귀한다(RQ-21 v1.4 마지막 문장)', () => {
    ...
    it('꼭대기 위(발판 있음) — 사다리 지배가 끝나고 그 지지 높이에서 접지 이동으로 전환된다. 같은 방향(이전엔 상승) 입력이 더 이상 y를 올리지 않고 대신 수평 이동을 일으킨다', () => {
      // 발판(topY = maxY)을 사다리 상단과 정확히 맞춘다 — 이 테스트는
      // **지지면이 있는** 이탈(접지 유지)만 다룬다. 지지면이 없는
      // 이탈(공중 전이·낙하 데미지, F1 수정)은 파일 하단의 "F1 재현"
      // describe가 별도로 고정한다(위 docblock "트레이드오프였으나 평가
      // FAIL로 지적돼 수정됨" 절 참고).
