...
- **검증 판정(평가·리뷰·QA·비판적 점검)은 반드시 별도 에이전트 세션에서 한다.**
  작업한 세션이 자기 산출물의 **합격 여부를 스스로 판정하지 않는다** — 컨텍스트를
  공유하는 순간 자기 채점이 되고, 작업자의 맹점이 검증자에게 그대로 옮겨간다.
  판정 세션에는 데이터를 파일로만 전달하고 대화·의도 설명은 넘기지 않는다.
  (해당: `tdd-workflow`의 evaluator, `review-gate`의 reviewer, 그 밖의 모든 검증 요청)
  ↔ **자기 작업의 테스트·빌드 실행과 그 출력 첨부는 여기 해당하지 않는다 —
  오히려 의무다**(바로 아래 TDD 항목). 증거를 **만드는** 것은 작업자가, 그 증거로
  합격을 **판정**하는 것은 격리 세션이 한다. 설치·git 조작 같은 단순 실행도 제외.
- TDD — **선별 Red** (ADR-0011): Red-first는 `src/shared`·서버 판정 로직·결함
  재현에만 요구. 그 외(클라 모듈·배선)는 test-after 허용하되 테스트는 같은
  PR에 동반. 완료 주장에는 테스트 실행 출력을 증거로.
