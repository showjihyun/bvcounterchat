 ...
 * - **벽·플랫폼에서는 점프에서도 0건**이다(최소 `0.300000`). 일반 규칙은
 *   **점프로 `topY`를 넘을 수 있는 지오메트리만 취약**하다는 것이다 —
 *   `JUMP_HEIGHT`(1.0)보다 높으면 상승 중에도 차단 목록에 남아 `near*`가
 *   계속 발동하기 때문이다. 벽은 무한 높이, 플랫폼은 `topY`=4라 **구조적
 *   면제**이고, 박스는 최대 `topY`=0.7이라 전부 취약하다. `eyeHeightM`=1.7 > 박스 최대 `topY`=0.7이므로
