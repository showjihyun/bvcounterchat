 ...
 * 가 성립할 수 없다(아래 §"정상 경로 간극 보존 증명" 참고). **팽창이
 * 겹쳐 무력해진 이음새에서만**, 실경계 절단이 몸이 그 AABB의 실제 내부로
 ...
 * 사라진다.
 *
 * **정상 경로 간극 보존 증명**: 고립된 AABB(또는 2R 이상 떨어진 쌍)에서
 ...
 * 절단이 이어받는다). 즉 정상 경로의 `R` 간극(24af·24ao 테스트가 고정)은
 * 조금도 줄지 않는다 — 실경계 절단은 팽창이 겹쳐 `near*`가 무력해진
 * 이음새에서만 관측 가능하다. */
