 ...
 * 합법 출발점 12,723개 × 8방향 × 3속도 × 50틱, `PRODUCTION_GEOMETRY`):
 *
 * - **무점프 경로에서는 완전히 보존된다** — 박스 위반 **0건**, 최소거리
 *   정확히 **0.300000**. 벽도 0건.
 * - **점프 경로에서는 보존되지 않는다** — 박스 위반 **4,124건**, 대상은
 *   프로덕션 박스 **15개 전부**, 최소거리 **0.000000**. 기전은 이음새가
 ...
 * - **벽에서는 점프에서도 0건**이다(최소 `0.300000`) — 벽은 무한 높이라
 *   `near*`가 항상 발동한다. `eyeHeightM`=1.7 > 박스 최대 `topY`=0.7이므로
