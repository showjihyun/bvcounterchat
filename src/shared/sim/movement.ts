 ...
 * 클램프와 정확히 같은 식이다. **정상 경로(고립된 AABB, 또는 2R 이상
 * 떨어진 쌍)에서는 이 실경계 절단이 항상 무력하다** — `near*` 절단이
 ...
 *
 * **그럼에도 결론이 서는 이유는 도달 가능성이다**: 프로덕션 박스 15개 ×
 * 3속도 + 점프 전수 측정에서 **띠 안에 착지하는 경로가 0건**이고 최악
 * 여유가 **+0.29m**(박스 #4, `topY=0.35`, 앉은 자세)다. 즉 24af·24ao의 `R`
 * 간극은 **도달 가능한 경로에서** 보존되며, 그것이 테스트가 고정하는 범위다.
 * ⚠️ **그 0.29m 여유를 지키는 가드는 없다** — 박스 높이·`bodyRadiusM`·이동
 * 속도를 만지면 조용히 잠식될 수 있다(원장 24at). */
function clampAgainstWalls(
  ...
  for (const wall of walls) {
    ...
    // 절 참고) — near가 팽창 겹침으로 무력해진 이음새에서만 발화하는
    // 최후 방어선. 정상 경로에서는 위 near 절단이 이미 x를 nearMinX 등에
    // 붙들어 뒀으므로 이 블록의 교차 조건이 성립하지 않는다(간극 불변).
