...
/** 네트워크 (RQ-60, RQ-64) */
export const NET = {
  /** 게임 서버 기본 포트. `PORT` 환경변수가 있으면 서버는 그쪽을 쓴다
   * (`src/server/index.ts`) — 이 값은 **기본값의 단일 출처**다.
   *
   * 클라이언트도 이 값을 읽는다(`@client/net/endpoint`) — 개발 모드에서는
   * Vite(5173)가 룸 WebSocket 경로를 프록시하지 못해 게임 서버로 직접 붙기
   * 때문이다(원장 24e-1). 양쪽이 각자 `2567`을 적으면 한쪽만 바꿨을 때
   * 조용히 어긋난다(ADR-0010 값 복제 금지). */
  DEFAULT_SERVER_PORT: 2567,
  /** 서버 고정 틱 레이트 (RQ-60) */
  TICK_HZ: 30,
