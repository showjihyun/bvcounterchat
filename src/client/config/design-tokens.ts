...
export const SCENE = {
   ...
   *
   * 4m를 고른 근거: 현재 맵에서 **도달 가능한 최고 지점이 사다리 꼭대기 4m**
   * (`LADDER_ALPHA.maxY`)다. 그보다 낮으면 사다리를 다 오른 플레이어가 벽
   * 윗면보다 높이 서서 "벽 밖이 보이는데 못 나간다"가 되고, 그보다 높이면
   * 확인할 방법이 없는 여분이다. **스펙에 벽 높이가 없어**(RQ-30은 플레이
   * 면적만 규정) 이 값은 스펙 값이 아니라 렌더 선택값이다 — 더 높은 사다리·
   * 박스가 들어오면 함께 올려야 하며, 그 결합은 원장 24g로 이월돼 있다.
   */
  wallRenderHeightM: 4,
} as const
