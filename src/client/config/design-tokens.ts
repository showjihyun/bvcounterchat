...
/**
 * 사격 연습용 과녁(RQ-34, 원장 24cw) — 네 벽 안쪽면에 그리는 동심원 표식.
 * 배치 산술(안쪽면 판정·중심 좌표·회전)은 `@client/scene/targetLayout`이 하고,
 * 이 토큰은 그 결과를 그릴 지오메트리·색만 정한다.
 *
 * ⚠️ **개수(벽마다 하나)·형태(동심원)·중심 높이(눈높이)만 RQ-34가 정한다** —
 * 링 반지름·색·표면 오프셋은 스펙에 근거가 없는 디자인 선택값이다(RQ-30/32/33이
 * 좌표·치수를 문면에 두지 않는 것과 같은 관례). 색은 새로 발명하지 않고
 * `COLOR.fgStrong`/`COLOR.danger`를 재사용한다(`HIT_MARKER`가 `COLOR.danger`를
 * 재사용한 것과 같은 이유 — 벽(`SCENE.wall`, 탄색) 위에서 대비가 커야 한다).
 */
export const TARGET = {
  /** 링 반지름(m), 바깥→안쪽. 인접한 두 값이 링 밴드 하나(바깥·안쪽 반지름)를
   * 이루고, **마지막 값은 밴드가 아니라 중심 원(불스아이)의 반지름**이다 —
   * 그래서 `ringColors`와 길이가 같다(밴드 3개 + 중심 원 1개 = 4). */
  ringRadiiM: [0.5, 0.35, 0.2, 0.08],
  /** 링 색 — `ringRadiiM`과 같은 길이, 바깥→안쪽. */
  ringColors: [COLOR.fgStrong, COLOR.danger, COLOR.fgStrong, COLOR.danger],
  /**
   * 벽 표면에서 띄우는 거리(m) — z-fighting 방지. ⚠️ **`DECAL.offsetM`과 같은
   * 크기가 아니라 그보다 작아야 한다 — 층 규칙이다: `0 < TARGET.offsetM <
   * DECAL.offsetM`.** 과녁은 벽과 탄흔 **사이 층**이다. RQ-34 문면이 「어디에
   * 맞혔나를 알려주는 것은 탄흔이지 과녁이 아니다」라고 못박았으므로, 같은
   * 벽면에 과녁과 탄흔이 겹치는 자리에서는 **탄흔이 과녁 위에** 그려져야 한다
   * (PR #90 리뷰 blocker B1 — 이전에는 두 오프셋이 우연히 같은 0.01이라
   * 두 평면이 부동소수 오차 안에서 완전히 겹쳤고, 어느 쪽이 이기는지가
   * `GameScene.tsx`의 컴포넌트 마운트 순서(=material 생성 순서, three
   * `painterSortStable`의 동률 처리)라는 **우연**에 맡겨져 있었다 — 그 순서가
   * 바뀌면 탄흔이 과녁 뒤로 사라진다).
   *
   * 값 0.005(5mm) 근거: 카메라 `near 0.1`/`far WORLD.SIZE_M*2`(`GameScene.tsx`),
   * 24비트 깊이 버퍼에서 깊이 분해능이 벽 근접 거리(15m)에서 ≈0.13mm,
   * **플레이 영역 대각 최장 시선(≈53m)에서 ≈1.65mm**다. 5mm면 그 최악에서도
   * 벽과 **≈3배**, 탄흔의 `DECAL.offsetM`=0.01과도(0.005 차이) 싸우지 않는다.
   *
   * ⚠️ **초판이 「방 대각 ≈35m → ≥6.8배」로 적었고 그것은 과소평가였다**
   * (PR #90 델타 재리뷰 minor m3 — 그 수치는 리뷰어의 1차 처방 문구를 그대로
   * 옮긴 것이고 리뷰어가 스스로 정정했다). **네 벽은 닫힌 방을 만들지 않는다** —
   * 폭 10m 판 4개일 뿐이고 플레이 영역은 60×60m다. 실제 최악은 코너
   * (≈(−29,−29))에서 EAST 과녁을 보는 **≈52.7m**다. **결론(0.005 유효)은
   * 바뀌지 않는다** — 3배도 1보다 크고, 같은 거리대에서 `DECAL.offsetM`=0.01이
   * 동작하는 것이 PR #83 시각 확인으로 관측됐다. 정정하는 이유는 **나중에 값을
   * 더 줄이려는 사람이 「6.8배나 남는다」로 읽는 것**을 막기 위해서다.
   *
   * `DECAL.offsetM`을 올려 맞추는 안은 **기각**됐다 — 그 상수는 RQ-71 피격
   * 효과와 같은 코드 경로를 공유해 과녁과 무관한 표현이 함께 움직인다
   * (`harness/workflow/fe.md` "효과 처리") — RQ-34 스코프 밖.
   */
  offsetM: 0.005,
  /** 원 세그먼트 수 — `HitDecals.tsx`의 탄흔·피격 원(12)보다 크다. 과녁은
   * 화면에 크게(최대 반지름 0.5m) 보이므로 세그먼트가 적으면 다각형 각짐이
   * 눈에 띈다. */
  segments: 32,
} as const
