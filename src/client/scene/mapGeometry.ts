 ...
 *
 * ⚠️ **판정과 표현이 여기서 갈라지고, 그 괴리는 이미 눈에 보인다**(PR #61
 * 리뷰 major). 판정상 벽은 무한 높이인데 렌더는 유한하다. 사다리 꼭대기에 선
 * 플레이어의 **시점은 4 + 1.7(눈높이) = 5.7m**라 그려진 벽 윗면보다 위에 있다 —
 * 벽 너머가 보이지만 그쪽으로 쏜 탄은 보이지 않는 기둥에 막힌다. 처분은 원장
 * 24g가 갖는다(값을 올릴지 ADR-0013을 유한 벽으로 개정할지).
 */
