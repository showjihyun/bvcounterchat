...
export class GameRoom extends Room<GameState> {
  ...
  private initializePlayer(sessionId: string, nickname: string): void {
    ...
    // `pendingSeqs`도 함께 지운다 — `respawnPlayer`가 이 맵을 보존하는
    // 이유(ADR-0003, "이미 플레이어였던 세션이 서버 처리 진행 상황을
    // 계속 추적")는 여기 그대로 적용되지 않는다: 이 메서드로 플레이어가
    // 되는 세션은(최초 입장이든 승격이든) 이 시점 이전에 플레이어였던
    // 적이 없다(승격 대상은 관전자만이었다 — 플레이어→관전자 강등 경로가
    // 없다). 그러니 보존해야 할 "이전 플레이어 시절의 진행 상황"이 없고,
    // 관전 중 보낸 합성 seq를 남겨두면 승격 직후 클라이언트가 보내지 않은
    // seq가 `lastProcessedInputSeq`로 확정 브로드캐스트돼 클라 예측
    // (RQ-62) 재조정 버퍼가 혼란을 일으킬 수 있다.
    this.pendingInputs.delete(sessionId)
    this.pendingSeqs.delete(sessionId)
