...
export class GameRoom extends Room<GameState> {
  ...
  private stepPlayerMovement(currentTick: number): void {
    ...
    this.state.players.forEach((player, sessionId) => {
      ...
      // 않는데도 이 리셋은 그대로 실행된다.
      //
      // **이 리셋은 선택이 아니라 필수다.** 실 클라이언트는 키를 누르고
      // 있어도 `jump:true`를 **키다운당 정확히 한 번만** 보낸다 —
      // `src/client/input/movementInput.ts`의 `getMoveInput()`이
      // `jumpPending`을 첫 호출에 소비하고 즉시 내리며, 그 파일 docblock이
      // "그렇지 않으면 착지 직후 키를 계속 누르고 있는 것만으로 자동 연속
      // 점프가 발생한다"고 그 의도를 명시한다. 따라서 위 'move' 핸들러의
      // OR 병합만 넣고 이 리셋을 빼면 **한 번의 키다운이 영원히 남아 무한
      // 버니합**이 된다(`22f-jump-input-loss.test.ts`의 "재발화 금지"
      // 케이스가 정확히 그 회귀를 잡는다 — 리셋만 제거하면 그 케이스가
      // 단독으로 `AssertionError`로 죽는 것을 독립 평가가 변이로 실증했다).
      //
