...
- 한 호출에서 최대 `maxTicksPerAdvance`틱만 진행한다. 남은 밀림은 누적기에
  ...
  스텝 유실 없이 보존**된다.
- 누적 밀림이 `maxBacklogTicks`를 **넘으면**(초과 시에만, == 는 정상 캐치업)
  그 호출의 **밀림 전량을 버린다**: 전진 0틱, `onOverflow(버린 전체 틱 수)`,
  백로그 0에서 재출발 — 다음 33.33ms 경과에서 정확히 1틱. (2026-07-24 확정:
  초과분만 버리고 남은 상한치를 따라잡으면 그 캐치업이 또 빨리감기가 되어
  "빨리감기 없이 유실"과 모순 — test-writer 분석, 사용자 결정.)
  **긴 정지(OS 서스펜드 등) 후 그 구간만 유실**하고 시뮬레이션은 현재 실
  시간 기준으로 재정렬된다.
