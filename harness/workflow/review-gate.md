...
| 항목 | 내용 | 근거 | 기본 심각도 |
|---|---|---|---|
| 스코프 이탈 | 스펙 밖 기능 추가 (팀전·구매 시스템·Pistol 외 무기·계정/로그인·모바일 조작·음성 채팅·매치메이킹·안티치트 등) | requirements.md §11 비요구사항 | blocker |
| ADR 모순 | 변경이 승인 ADR(0001~0009)과 모순 | 해당 ADR 번호 | blocker |
| 서버 권위 위반 | 클라이언트가 위치·HP·킬 등 게임 상태를 최종 결정하는 코드 (서버 응답을 그대로 신뢰하지 않고 클라이언트 계산값을 권위로 사용) | RQ-61 | blocker |
| 렌더 루프 내 할당 | `useFrame`/렌더 루프 안에서 매 프레임 벡터·객체·배열을 새로 생성 | `fe.md` 프레임 예산 규칙 | major (반복적·명백하면 blocker로 격상) |
| 테스트 약화 | 테스트 diff에 기대값 완화·케이스 삭제·skip 추가 | evaluator 교차 감시 | blocker |
| 문서 동행 | 스펙·ADR 변경 이력이 코드와 같은 PR에 있는가 | CLAUDE.md 최상위 규칙 | major |
