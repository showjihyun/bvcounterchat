...
**minor를 같은 PR에서 고치는 것은 기본값이 아니다.** 원장
(`harness/progress.md`) 후속 항목으로 넘기는 것이 기본이고, in-PR 수정은
다음 둘 중 하나일 때만 한다:
...
그 외(설계 제안, 관측된 적 없는 취약성, 스타일)는 전부 이월한다. 이월은
누락이 아니다 — 원장 행에 **근거와 수정안까지** 적는다.

리뷰어가 "**즉시 조치 불요**"로 판정한 항목은 오케스트레이터가 되묻지 않고
자동 이월한다.

### 재리뷰 조건 (ADR-0012)

- blocker·major를 고쳤다 → **델타 재리뷰 필요**
- **minor만 반영했다 → 재리뷰 없이** `npm run check` + CI 통과로 종결
...
리뷰가 **전부 통과시킨** 통합 테스트 타이밍 결함이었다. reviewer는 결정론
검사를 대신할 수 없고, 반대로 결정론 검사를 통과한 코드만 추론 검사를 받을
가치가 있다.
