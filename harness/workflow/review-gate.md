...
위에서 났다(원장 28af). ⚠️ **위 기전은 일반론이고 이 저장소에는 성립하지
않는다** — `allow_auto_merge`가 **꺼져 있어**(오늘도 불변) `--auto`는 보호
여부와 무관하게 즉시 머지로 퇴화한다. **보호를 건 지금도 `--auto`는 금지다**
(델타 재리뷰 minor E4 — 바로 윗줄이 「보호를 걸었다」라 「이제 써도 되겠다」로
이어 읽힐 여지가 있었다).
