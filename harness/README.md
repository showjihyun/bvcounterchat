...
지금 **기계가** 막는 것: 🟡가 남은 상태의 구현 착수(hook + CI),
lint·typecheck·test 회귀(`npm run check` + CI). 머지 전 독립 리뷰는 필요조건이지만
아래에 적은 대로 **규율로만** 지켜진다.
...
**GitHub 브랜치 보호는 2026-08-10에 걸렸다**(원장 28af — 설정 실측값의 정본은
그 행이다). `gate`가 필수 검사가 되어 **CI 미통과 머지는 이제 기계가 막는다**.
⚠️ **리뷰 승인 요구는 걸지 않았다**(솔로 체제라 걸면 자기 PR을 못 머지한다) —
따라서 **"reviewer APPROVE 없이 머지 금지"도 "테스트 커밋이 먼저"도 여전히
규율로만** 지켜진다. 스캐폴딩 후 3주간 보호가 **없었는데 `review-gate.md`는
있다고 단언**하고 있었고, 그 어긋남 위에서 PR #76의 머지 사고가 났다.
