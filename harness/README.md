...
| Sensor | 실행체 | 무엇을 막나 |
|---|---|---|
| 스펙 동결 게이트 | `.claude/hooks/gate_spec_freeze.py` + CI | 🟡가 남은 상태의 구현 착수 |
| 결정론적 검증 | `scripts/check.sh` (게이트→lint→typecheck→test) + CI | 회귀·타입 오류·lint 위반 |
| 리뷰 게이트 | `.claude/skills/review-gate/` + `agents/reviewer.md` | 스코프 이탈·ADR 모순·서버 권위 위반 |
...
**여전히 없는 것.** 트래젝토리 로그, 골든 파일 ask 게이트(17g), GitHub 브랜치
보호. 즉 "reviewer APPROVE 없이 머지 금지"는 지금 **규율로만** 지켜진다.
