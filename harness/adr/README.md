...
| [ADR-0016](0016-hit-feedback-authority.md) | 명중 피드백의 권위 | **서버가 명중 이벤트를 보낸다**(좌표·법선·대상 종류) — ⚠️ ADR-0014(발소리는 클라 로컬)와 **반대 방향**이고 갈린 이유는 **랙 보상**이다(서버가 RQ-64로 되감아 판정하므로 클라 재현은 화면과 판정이 갈린다, 최대 1.2m) · 탄흔은 **상한 64로만** 제거하고 시간으로 안 사라진다 · 피격은 **일시적** · 정보 누출 필터를 두지 않은 것은 이 라운드의 선택이고 그 무효 조건은 원장 **24ca**가 소유한다 | 승인 |
