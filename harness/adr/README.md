...
| [ADR-0016](0016-hit-feedback-authority.md) | 명중 피드백의 권위 | **서버가 명중 이벤트를 보낸다**(좌표·법선·대상 종류 **+ 사수·피격자 식별자**, 2026-08-15 개정) — ⚠️ ADR-0014(발소리는 클라 로컬)와 **반대 방향**이고 갈린 이유는 **랙 보상**이다(서버가 RQ-64로 되감아 판정하므로 클라 재현은 화면과 판정이 갈린다, 최대 1.2m) · 탄흔은 **상한 64로만** 제거하고 시간으로 안 사라진다 · 피격은 **일시적** · 정보 누출 필터를 두지 않은 것은 이 라운드의 선택이고 그 무효 조건은 원장 **24cc**가 소유한다 ⚠️ **2026-08-15 개정(원장 24ct)** — **결정 1에 사수·피격자 식별자 추가**. 관측자가 자기와 사건의 관계를 알 수 없어 히트마커(RQ-57)·피격 방향 표시(RQ-58)·카메라 반응(RQ-59)이 정본대로 구현 불가였다. 클라는 **「나와의 관계」만 가르고** 판정은 여전히 서버다(RQ-61). ⚠️ **정보 누출이 는다** — RQ-56이 「상시 이름표」를 버린 근거와 같은 계열이라 **RQ-31 캠핑 방지를 의식적으로 후퇴**시켰다. **무효 조건**을 결정 1에 걸었고 소유는 원장 **24cc**다. **결정 2는 범위 한정** — 순간 피격 효과(TTL)와 **표면 splatter(상한)**를 다른 것으로 갈랐다. 상한값·파티클은 **RQ-71 개정**(구현 라운드, post-gate)이 정한다 | 승인 |
