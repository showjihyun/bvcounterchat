...
착수 시 레이아웃 결정이 없다는 것이 드러나 ADR-0010을 신설했다. 이후 라운드를 돌리며 ADR-0011(선별 Red)·ADR-0012(검증 꼬리 간소화)가, 그리고 로드맵 8단계(맵) 착수 직전 **ADR-0013**(이동 정본과 충돌 계층의 분리 — ADR-0004 결정 1·3 대체)이 더해져 **총 13건**이다. 이후 구현 라운드를 돌리면서 **ADR-0011**(선별 Red)·**ADR-0012**(검증 꼬리 간소화)를 추가해 총 **12건**이다. 답변 근거는
