...
| [ADR-0015](0015-player-model-geometry.md) | 캐릭터 모델의 형상과 권위 | **절차적 지오메트리**(모델 에셋 파일 0개) · **히트박스가 정본**(모델이 히트박스를 따르며 그 역이 아니다 — 볼륨은 **원통 ∪ 구체**이지 외접 원통이 아니다) · **앞뒤 대칭**(yaw 미동기의 대가 — `GameState.ts`에 yaw를 올리면 **무효**이고 원장 **24cb**가 소유한다) · 걸음 위상은 **RQ-72와 별개 누적**(발소리는 달리기만, 스윙은 모든 접지 이동) | 승인 |
