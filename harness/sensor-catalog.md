...
| 독립 평가 에이전트 (evaluator) | Inf | 각 RQ 구현 직후 (tdd-workflow Phase 3) | 오케스트레이터 스킬 `.claude/skills/tdd-workflow/SKILL.md` | ✅ **2026-07-21 구축.** `.claude/agents/evaluator.md`(opus). 검증 항목 6건(스위트 재실행·골든 커버리지·Colyseus 경계면 필드 대조·결정론·테스트 약화·스코프). 격리: RQ-ID와 `_workspace/{RQ-ID}/` 경로만 받고 coder 대화는 받지 않는다 — 자기 채점 방지 |
