...
| 17e | **결정론 시뮬레이션 하네스 (고정 틱 + 시드 RNG + 틱 스케줄러)** | RQ-11/15/16/43/60/90, ADR-0008 | ✅ | `src/shared/sim/{clock,rng,scheduler}.ts`, `tests/support/harness.ts` | **완료(2026-07-21)**. evaluator **PASS**. 테스트 52건 신규(총 67건 통과), 커밋 순서 테스트→구현 확인, 테스트 약화 0. **핵심 판단**: 시간의 정본 단위는 틱(정수) — `ticksToMs(60)`이 `2000.0000000000002`인 것이 실측으로 확인돼 ms 누적 금지의 근거가 됐다. RNG는 32비트 정수 연산(`Math.imul`/`>>> 0`), 스케줄러는 이진 최소 힙(틱 경로 O(log n)). 평가 중 검출된 lint 구멍(`setTimeout` 금지가 강제되지 않음) 즉시 수정 |
| 17e-1 | 하네스 후속 (평가 관찰 3건) | ADR-0008/0010 | ⬜ | `src/shared/sim/`, `eslint.config.js` | **PR #4 evaluator 관찰에서 이월.** ① `createRng(1.5)`가 `createRng(1)`과 **같은 스트림**을 준다(`seed >>> 0` 조용한 강제) — `advance`·`scheduleAt`에는 "조용한 반올림 금지"를 적용해놓고 생성자 경로만 비대칭이다. 시드를 계산식으로 만들 때 무증상 충돌 위험. ② `assertNonNegativeInteger`가 `clock.ts`·`scheduler.ts`에 중복 정의 — 한쪽만 고치면 에러 의미가 갈리고 게이트가 못 잡는다. ③ `advanceTo` 재진입·`AggregateError` 후 힙 일관성은 계약에 없어 미검증 |
| 17e-2 | 하네스 플랫폼 간 RNG 동일성 검증 | RQ-62 | ⬜ | — | **미확인 이월.** 32비트 정수 연산만 쓴다는 건 소스로 확인됐으나 다른 엔진(브라우저 JS, ARM Node)에서 같은 수열이 나오는지는 단일 환경(Node/win32)에서 검증 불가. 기준값이 `03_evaluator_report.md` §4-2 B1에 남아 있다 — **클라이언트 코드가 생기면 그때 대조한다.** 갈라지면 클라이언트 예측(RQ-62)이 서버와 어긋난다 || 17f | coder 세션의 `tests/` 쓰기 차단 hook (후속) | 파이프라인 무결성 | ⬜ | `.claude/hooks/` (미생성) | **PR #2 리뷰 blocker 2 후속 권고.** coder는 `Edit`·`Bash` 권한이 있어 테스트 파일 수정이 도구 수준에서 막히지 않는다. 현재 방어는 evaluator의 사후 diff 검출 하나뿐 — `gate_spec_freeze.py`가 PreToolUse 경로 차단의 선례이므로 같은 방식으로 프롬프트 강제를 결정론적 게이트로 승격 가능 |
