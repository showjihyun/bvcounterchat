...
| 19a | **RQ-04 상설 세션 (파이프라인 첫 RQ 실전)** | RQ-04, ADR-0002 | 🔄 | `feat/RQ-04-persistent-session` 브랜치 | **착수(2026-07-22)**. `tdd-workflow` 스킬의 첫 정식 RQ 적용(하네스 3건은 스킬 우회였음). Phase 0에서 두 전제 막힘 실측·해소: ① RQ-04 매핑 GA 케이스 0건 → **사용자가 검증 기준 결정**(GA-26~29 신설, 정답은 사람이 씀). ② colyseus.js 미설치 + schema 메이저 불일치 → 0.16 라인 정렬. GA-26(0명이어도 유지)·GA-27(라운드 종료 없이 재입장)·GA-28(재시작 시 소실)·GA-29(단일 세션) |
