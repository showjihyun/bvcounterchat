...
| 20a | **RQ-60 30Hz 고정 틱 (3단계 착수)** | RQ-60, ADR-0003/0008, 17e-2 일부 해소 | 🔄 | `feat/RQ-60-fixed-tick` 브랜치 | **착수(2026-07-22)**. GA-14. 결정론 하네스(clock·scheduler)가 실 서버 30Hz 루프에 처음 결합되는 지점 — 하네스 계약의 "서버는 실 루프로, 테스트는 수동으로 같은 인터페이스" 설계가 여기서 검증된다. 테스트 전략: 고정 스텝 누적기 로직은 결정론(하네스), 실 루프는 관대한 실측 최소화(flaky 방지) |
