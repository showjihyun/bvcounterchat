...
| 19b-1 | 닉네임 새니타이즈 (RQ-02 리뷰 major 이월) | RQ-02, RQ-40/51/55 표시 계층 | ⬜ | `src/server/rooms/GameRoom.ts` | **PR #7 리뷰 major — reviewer가 후속 타이밍 지정(RQ-40/HUD 착수 전 필수).** 닉네임이 길이·문자 제한 없이 권위 상태에 저장·브로드캐스트된다: ① 길이 상한 없음(ws 기본 100MB까지 통과 → 전원 브로드캐스트 증폭) ② 제어문자·개행 통과 → Top Scorer/킬피드/채팅 표시를 깨거나 시각적 사칭 가능. 객체/배열 주입은 `typeof==='string'` 필터로 이미 차단됨. 수정 방향: `uniqueNickname` 호출 전 길이 클램프+제어문자 제거+트림 폴백. **관전자(RQ-03)도 동일 무검증 경로를 탄다** — 수정 지점이 플레이어/관전자 분기 위 공유 지점이므로 구조적으로 동시 커버되지만, 구현 시 관전자 케이스 테스트를 빠뜨리지 말 것(PR #8 리뷰 minor). 스펙이 길이·문자에 침묵하므로 값 결정 시 스펙 개정 동반(코드와 같은 PR) |
