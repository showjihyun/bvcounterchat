...
| 17e | **결정론 시뮬레이션 하네스 (고정 틱 + 시드 RNG + 틱 스케줄러)** | RQ-11/15/16/43/60/90, ADR-0008 | 🔄 | `src/shared/sim/{clock,rng,scheduler}.ts`, `tests/support/harness.ts` | 테스트 **56건 신규(총 70건 통과)**. **핵심 판단**: 시간의 정본 단위는 틱(정수) — `ticksToMs(60)`이 `2000.0000000000002`인 것이 실측 확인돼 ms 누적 금지의 근거가 됐다. RNG는 32비트 정수 연산, 스케줄러는 이진 최소 힙. **⚠️ 게이트 이력(기록 가치 최상)**: evaluator가 1차 **PASS**를 냈으나 그 뒤 리뷰 게이트가 **blocker**를 검출했다 — `advanceTicks(n)`이 시계를 벌크로 옮겨 벌크 경로와 틱별 경로가 다른 결과를 냈다(서버는 틱별, 테스트는 벌크 → 하네스로 통과한 테스트가 서버 동작을 보장하지 못함). **evaluator 잘못이 아니라 계약 §4의 누락이 원인**이었고, 계약에 「전진 단위 불변식」을 추가한 뒤 `dcd4f87`(수정)·`f79b7c4`(회귀 테스트 3건)로 대응, 재평가 PASS(변이 테스트로 회귀 검출력까지 확인). **평가 게이트가 샜다는 사실 자체가 이 항목의 핵심 기록이다** |
| 17e-1 | 하네스 후속 — PR #4 리뷰 major 3건 (미수정) | ADR-0008/0010, RQ-60 | ⬜ | `src/shared/sim/`, `_workspace/review/chore-determinism-harness.md` | **PR #4 리뷰 major 4건 중 1건만 수정(17f 병합 복구). 나머지 3건이 여기 남는다.** ① **`advanceTo` 무제한 루프** — 콜백이 `scheduleIn(0)`으로 자기를 재예약하면 같은 `advanceTo` 안에서 끝없이 pop된다. evaluator 실측: 상한 가드를 걸어 재보니 `advanceTicks(1)` 한 번에 **콜백 200,000회**(상한값), 가드 없으면 무한. 틱별 루프 전환 뒤에도 그대로다 — **서버 틱 경로(RQ-60)의 행(hang) 위험**. 호출자 버그이지만 서버에서는 곧 정지다. ② **`createClock(startTick)` 인자 무검증** — `advance`·`scheduleAt`에는 "조용한 반올림 금지"를 적용해놓고 생성자만 비대칭. ③ **`createRng(seed)`·`fork(salt)`의 `seed >>> 0` 조용한 강제** — `createRng(1.5)`와 `createRng(1)`이 **같은 스트림**이다. 시드를 계산식으로 만들 때 무증상 충돌. 함께: `assertNonNegativeInteger`가 `clock.ts`·`scheduler.ts`·`harness.ts`에 3중 중복. ④ **계약 미규정(재평가 r3 관측)**: 연쇄 폭주 `RangeError`로 즉시 중단할 때 이미 `errors`에 쌓인 콜백 예외가 버려진다 — 상한 초과와 콜백 예외의 조합을 계약이 규정·테스트하지 않았다. 계약 보강 후보 |
