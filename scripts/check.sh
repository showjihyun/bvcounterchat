...
# 약 10일 남아 있었다. todo였으므로 "검증됨"으로 읽힌 적은 없다. "처리했다는
# 보고가 검증 없이 나간다"는 같은 계열의 드리프트에 사람 주의력이 다섯 번째로
# 졌다는 원장 28e의 취지는 여전히 유효하다.
# ✅ 원장 28i(사용자 결정) — todo도 verify가 깨져 있으면 이제 경고를 낸다
# (A-③, 하드 실패는 아니다) — GA-52가 겪은 바로 그 형태(todo 장기 체류)를
# 닫는다. 상세: gate_golden_backref.py 상단 docblock.
