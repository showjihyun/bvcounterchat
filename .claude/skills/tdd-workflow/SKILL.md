...
- **PASS** → 구현 커밋 확인, 원장(`progress.md`) ✅ 갱신, PR 준비
  ...
  **`review-gate` 스킬을 호출한다** — reviewer APPROVE가 머지의 필요조건이다.
  CI가 빨간 상태에서 리뷰를 시작하지 않는다.
  **PASS 이후 §2(b)(1줄급·회귀 위험 0) 수정만 반영한 경우 델타 재평가를
  돌리지 않는다 — 심각도와 무관하다**(ADR-0012 결정 3: 재평가를 정하는 것은
  **수정 등급 축**이고 재리뷰를 정하는 것은 **심각도 축**이다. 따라서 `major+(b)`는
  재리뷰만 간다). 단 **계약·스펙 위반 수정(a)** 이나 **CI 실패 대응 수정**이
  포함됐으면 델타 재평가는 여전히 필요하다 — 둘 다 실측상 "기존 테스트가
  구분하지 못하는 변경"이거나 "회귀를 실제로 만든" 경로다.
  재리뷰 판정은 `review-gate` 스킬이 하며 그 표가 정본 미러다.
