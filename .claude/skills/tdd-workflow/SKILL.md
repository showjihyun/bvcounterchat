...
- **PASS** → 구현 커밋 확인, 원장(`progress.md`) ✅ 갱신, PR 준비
  ...
  **PASS 이후 §2(b)(1줄급·회귀 위험 0) minor만 반영한 경우 델타 재평가를
  돌리지 않는다**(ADR-0012 결정 3). 단 **계약·스펙 위반 수정**이나 **CI 실패
  대응 수정**이 포함됐으면 델타 재평가는 여전히 필요하다 — 둘 다 실측상
  "기존 테스트가 구분하지 못하는 변경"이거나 "회귀를 실제로 만든" 경로다.
